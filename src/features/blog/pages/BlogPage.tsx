...
export default function BlogPage() {
  ...
  const allCategories = [{ id: 'all', name: 'Всі' }, ...categories];
    ...
    <div className="min-h-screen bg-farm-cream/30">
      ...
      <main className="max-w-7xl mx-auto px-4 pt-32 pb-24">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
          <div>
            <h1 className="text-4xl md:text-5xl font-bold text-farm-green mb-4">Блог</h1>
            <p className="text-farm-wood opacity-70">Цікаві історії, рецепти та новини з нашої ферми</p>
          </div>
                    ...
                    <div className="flex gap-2 mb-4">
                      {article.categoryId && (
                        <span className="text-[10px] font-bold uppercase tracking-wider text-white px-2 py-1 bg-farm-green rounded-md">
                          {categories.find(c => c.id === article.categoryId)?.name || 'Блог'}
                        </span>
                      )}
