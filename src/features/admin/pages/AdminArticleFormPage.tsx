...
function EditorToolbar({ editor, onInsertProduct, onUploadImage }: { editor: any, onInsertProduct: () => void, onUploadImage: () => void }) {
...
export default function AdminArticleFormPage() {
  ...
  const handleEditorImageUpload = async () => {
    ...
    input.onchange = async (e: any) => {
      ...
      try {
        ...
        showToast({ message: 'Зображення вставлено', type: 'success' });
      } catch (err) {
        showToast({ message: 'Помилка завантаження зображення', type: 'error' });
      } finally {
        setSaving(false);
      }
    };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (!formData.title.uk?.trim() || !formData.excerpt.uk?.trim() || !formData.body.uk?.trim() || formData.body.uk === '<p></p>') {
      showToast({ message: 'Заголовок (UA), короткий опис (UA) та текст статті (UA) обовʼязкові', type: 'error' });
      return;
    }
      ...
      <form onSubmit={handleSave} className="grid lg:grid-cols-4 gap-8">
        <div className="lg:col-span-3 space-y-8">
          <div className="bg-white p-10 rounded-[3rem] shadow-sm border border-gray-100 space-y-8">
            ...
            <div>
              <label className="block mb-2 font-medium">Текст статті <span className="text-red-500">*</span></label>
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-farm-wood opacity-50">
                          {typeof product.price === 'number' ? product.price : (product.price?.UAH || 0)} грн
                        </p>
                      </div>
